...
Use `npm ci` for application dependencies and `npm run check` as the integrated application gate. Run focused Vitest or Playwright checks while iterating. Packaging changes also require `npm run build` and `npm run verify:xdc`. Agent-policy or runtime changes require `task agent:team:generate`, `task agent:validate`, and `task agent:fixed-layout:test` before commit.
