...
function capEditorLines(lines: string[], count: number, width: number): string[] {
  ...
  const cursorRow = lines.findIndex(
    (line) => line.includes(CURSOR_MARKER) && normalizeLine(line, width).includes(CURSOR_MARKER),
  );
  if (cursorRow !== -1) {
  ...
  const selectedRow = lines.findIndex(
    (line) =>
      line.includes("→") &&
...
export function renderFixedEditorCluster(input: FixedEditorClusterInput): FixedEditorClusterRender {
  ...
  const editorSource = (input.editorLines ?? []).filter(
  ...
  return extractCursor(
    input.placement === "above"
      ? [...status, ...primary, ...editorLines, ...secondary, ...transcript, ...lastPrompt]
      : [...status, ...editorLines, ...primary, ...secondary, ...transcript, ...lastPrompt],
  );
}
