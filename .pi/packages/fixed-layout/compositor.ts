// Adapted from pi-powerline-footer's MIT-licensed fixed editor compositor:
// https://github.com/nicobailon/pi-powerline-footer/tree/b023235090525b1101a776f916ab546d25cf29a9
import {
...
export type ScrollAwayNavigationShortcutId =
  | "bottom"
  | "previousUser"
  | "nextUser"
  | "previousAssistant"
...
function compactShortcutPair(previous: string, next: string): string {
  ...
  const sameModifiers =
    previousShortcut.modifiers.length === nextShortcut.modifiers.length &&
    previousShortcut.modifiers.every(
      (modifier, index) => modifier.toLowerCase() === nextShortcut.modifiers[index]?.toLowerCase(),
    );
...
function buildScrollAwayCardCandidates(
  shortcuts: ReadonlyMap<ScrollAwayNavigationShortcutId, ScrollAwayNavigationShortcut>,
  ...
  if (previousUser && nextUser) {
    longMessageRows.push({
      kind: "content",
      left: "User messages",
      right: `prev ${previousUser.shortcutLabel} · next ${nextUser.shortcutLabel}`,
    });
    shortMessageRows.push({
      kind: "content",
      left: "User",
      right: `prev ${previousUser.shortcutLabel} · next ${nextUser.shortcutLabel}`,
    });
    compactMessageRows.push({
      kind: "content",
      left: "User prev/next",
      right: compactShortcutPair(previousUser.shortcutLabel, nextUser.shortcutLabel),
    });
  } else {
    if (previousUser) {
      longMessageRows.push({
        kind: "content",
        left: "Previous user",
        right: previousUser.shortcutLabel,
      });
      shortMessageRows.push({
        kind: "content",
        left: "User prev",
      ...
      compactMessageRows.push({
        kind: "content",
        left: "User prev",
        right: compactShortcutLabel(previousUser.shortcutLabel),
      });
    }
    if (nextUser) {
      ...
      compactMessageRows.push({
        kind: "content",
        left: "User next",
        right: compactShortcutLabel(nextUser.shortcutLabel),
      });
    }
  }
  ...
  if (previousAssistant && nextAssistant) {
    longMessageRows.push({
      kind: "content",
      left: "Assistant responses",
      right: `prev ${previousAssistant.shortcutLabel} · next ${nextAssistant.shortcutLabel}`,
    });
    shortMessageRows.push({
      kind: "content",
      left: "Assistant",
    ...
    compactMessageRows.push({
      kind: "content",
      left: "Asst prev/next",
      right: compactShortcutPair(previousAssistant.shortcutLabel, nextAssistant.shortcutLabel),
    });
  } else {
    if (previousAssistant) {
      longMessageRows.push({
        kind: "content",
        left: "Previous assistant",
        right: previousAssistant.shortcutLabel,
      });
      shortMessageRows.push({
        kind: "content",
        left: "Asst prev",
      ...
      compactMessageRows.push({
        kind: "content",
        left: "Asst prev",
        right: compactShortcutLabel(previousAssistant.shortcutLabel),
      });
    }
    if (nextAssistant) {
      longMessageRows.push({
        kind: "content",
        left: "Next assistant",
        right: nextAssistant.shortcutLabel,
      });
      shortMessageRows.push({
        kind: "content",
        left: "Asst next",
      ...
      compactMessageRows.push({
        kind: "content",
        left: "Asst next",
        right: compactShortcutLabel(nextAssistant.shortcutLabel),
      });
    }
  }
  ...
  if (compactRows.length > 0 && compactMessageRows.length > 0)
    compactRows.push({ kind: "divider" });
...
/** Records that the current renderer carries the compositor's own-property hooks. */
export const RENDERER_HOOK_MARKER = "__fixedLayoutRendererHooks";

function resolveInheritedFunction(
  target: object,
  property: string,
): ((...args: any[]) => any) | null {
...
export class TerminalSplitCompositor {
  ...
  constructor(options: TerminalSplitCompositorOptions) {
    ...
    this.outputPad = Number.isFinite(options.outputPad)
      ? Math.max(0, Math.floor(options.outputPad ?? 0))
      : 0;
    this.rowsDescriptor = descriptorForRows(options.terminal);
    this.originalWrite = options.terminal.write.bind(options.terminal);
    this.originalDoRender = this.captureTuiMethod<() => void>("doRender");
    this.originalRender = this.captureTuiMethod<(width: number) => string[]>("render");
  }

  /**
   * Captures the implementation that `install()` delegates to after it replaces
   * `tui[property]`.
   *
   * Pi 0.84 no longer hands extensions the renderer itself. It passes a Proxy
   * (`createInteractiveTuiReference`) so fullscreen mode can swap renderers
   * mid-session, and that Proxy's `get` trap returns a fresh closure which
   * re-reads `tui[property]` on every call. Binding that closure and then
   * assigning over `tui[property]` makes the replacement call itself until the
   * stack overflows, so resolve the inherited implementation instead. The
   * replacement is an own property, which prototype lookup skips, so resolving
   * per call also stays correct when fullscreen mode swaps the renderer.
   */
  private captureTuiMethod<T extends (...args: any[]) => any>(property: string): T | null {
    const tui = this.tui as object;
    const current = Reflect.get(tui, property);
    if (typeof current !== "function") return null;

    const own = Reflect.getOwnPropertyDescriptor(tui, property);
    if (own && typeof own.value === "function") {
      return own.value.bind(tui) as T;
    }

    const inherited = resolveInheritedFunction(tui, property);
    if (!inherited) return current.bind(tui) as T;

    return ((...args: unknown[]) =>
      (resolveInheritedFunction(tui, property) ?? inherited).apply(tui, args)) as T;
  }

  install(): void {
    if (this.installed) return;
    if (typeof this.terminal.write !== "function") {
      throw new Error("[fixed-layout] Expected terminal.write(data) to exist");
    }
  ...
  /**
   * Installs the renderer-level hooks on whichever renderer the TUI reference
   * currently points at.
   *
   * Pi 0.84's reference forwards assignment to the renderer that is current when
   * the write happens, so these land as own properties on that one renderer. A
   * TUI mode change builds a replacement renderer (`this.renderer = nextUi`) and
   * carries none of them over, and Pi offers no event for extensions to reattach.
   * `ensureRendererHooks` covers that from the terminal side, which survives the
   * swap because Pi reuses the terminal.
   */
  private attachRendererHooks(): void {
    ...
    if (typeof this.tui.compositeLineAt === "function") {
      this.originalCompositeLineAt = this.captureTuiMethod<CompositeLineAt>("compositeLineAt");
    ...
    // A plain value, so the reference returns it unwrapped instead of the
    // function wrapper its `get` trap builds for methods.
    this.tui[RENDERER_HOOK_MARKER] = true;
  }

  private ensureRendererHooks(): void {
    if (!this.installed || this.disposed) return;
    if (this.tui[RENDERER_HOOK_MARKER] === true) return;
    this.attachRendererHooks();
  }
    ...
    this.originalWrite(
      beginSynchronizedOutput() +
        ...
        buildFixedClusterPaint(
          this.decorateCluster(cluster, width),
          rawRows,
          width,
  ...
  dispose(options: DisposeOptions = {}): void {
    ...
    // Cleared by assignment, not deletion: Pi's reference forwards `set` to the
    // current renderer but has no `deleteProperty` trap, so a delete would hit
    // the reference's own empty target and leave the marker in place.
    this.tui[RENDERER_HOOK_MARKER] = false;
  ...
  private updateScrollBounds(scrollableRows: number): boolean {
    ...
    const changed =
      nextScrollOffset !== this.scrollOffset || this.maxScrollOffset !== previousMaxScrollOffset;
    ...
    if (mousePackets) {
      ...
      for (const packet of mousePackets) {
        ...
        this.handleMousePacket(packet, {
          skipScrollAwayCard: hadQueuedScroll || flushedQueuedScroll,
        });
      }
      if (wheelDeltas.length > 0) {
        this.queueScrollDeltas(wheelDeltas);
  ...
  private handleMousePacket(
    packet: SgrMousePacket,
    options: { skipScrollAwayCard?: boolean } = {},
  ): void {
    const delta = mouseScrollDelta(packet);
    if (delta !== 0) {
  ...
  private renderVisibleRootLines(start: number, width: number, scrollableRows: number): string[] {
    ...
    for (let index = 0; index < card.lines.length; index++) {
      ...
      renderedLines[row] = this.composeScrollAwayCardLine(
        renderedLines[row] ?? "",
        card.lines[index] ?? "",
        card.startCol,
        card.width,
  ...
  private composeScrollAwayCardLine(
    baseLine: string,
    overlayLine: string,
    startCol: number,
    overlayWidth: number,
    ...
    if (typeof this.tui.compositeLineAt === "function") {
      return sanitizeLine(
        this.tui.compositeLineAt(base, overlayLine, startCol, overlayWidth, totalWidth),
        totalWidth,
      );
    }
  ...
  private handleScrollAwayCardClick(packet: SgrMousePacket, width: number): boolean {
    ...
    if (
      !this.isScrollAwayCardClick(packet, width) ||
      !this.scrollAwayNavigationCard?.onClickBottom?.()
    )
      return false;
  ...
  private isScrollAwayCardClick(packet: SgrMousePacket, width: number): boolean {
    const card = this.computeScrollAwayNavigationCard(
      this.innerWidth(width),
      this.visibleScrollableRows,
    );
  ...
  private computeScrollAwayNavigationCard(
    ...
    const shortcutById = new Map(
      this.scrollAwayNavigationCard.shortcuts.map((shortcut) => [shortcut.id, shortcut]),
    );
  ...
  private scrollByDeltas(deltas: number[], options: { deferRender?: boolean } = {}): void {
    ...
    this.scrollOffset = nextOffset;
    this.pendingImageCleanup = true;
    this.repaintScrollableViewport(width, layout, previousScrollOffset);
  ...
  private scheduleScrollSettledRender(): void {
    ...
    if (
      typeof this.scrollSettledRenderTimer === "object" &&
      "unref" in this.scrollSettledRenderTimer
    ) {
  ...
  private repaintScrollableViewport(
    ...
    const contentWidth = this.innerWidth(width);
    const previousCard = this.computeScrollAwayNavigationCard(
      contentWidth,
      previousScrollableRows,
    ...
    let buffer = beginSynchronizedOutput() + this.consumePendingImageCleanup() + disableAutoWrap();
  ...
  private scheduleAlternateScreenKeyboardRetry(remainingAttempts: number): void {
    ...
    if (
      typeof this.alternateScreenKeyboardRetryTimer === "object" &&
      "unref" in this.alternateScreenKeyboardRetryTimer
    ) {
  ...
  private getCluster(width: number, terminalRows: number): FixedEditorClusterRender {
    ...
    const cluster = this.withClusterRender(() =>
      this.renderCluster(this.innerWidth(width), terminalRows),
    );
  ...
  private decorateCluster(
    ...
    const lines =
      this.selectionArea === "cluster"
        ...
        : cluster.lines;
    const paddedLines = this.overlaySelectionHint(lines, contentWidth).map((line) =>
      this.insetLine(line, width),
    );
    const padding = this.outerPadding(width);
  ...
  private overlaySelectionHint(lines: string[], width: number): string[] {
    ...
    const next = [...lines];
    next[next.length - 1] = this.composeScrollAwayCardLine(
      next[next.length - 1] ?? "",
      hint,
      width - hintWidth,
      hintWidth,
