...
function navigationCardOptions(
  onClickBottom: () => boolean = () => false,
): ScrollAwayNavigationCardOptions {
  return {
    shortcuts: [
...
test("terminal split applies outputPad as an outer fixed-editor inset", () => {
  ...
  assert.deepEqual(tui.render(12), [
    " root:10 ",
    "  ",
    "  ",
    "  ",
...
test("terminal split removes the old navigation card before shifting rows", () => {
  ...
  assert.ok(
    paint.indexOf("line-", 0) < shiftIndex,
    "old card rows should be restored before the terminal shift",
  );
  assert.ok(
    paint.indexOf("Jump to bottom") > shiftIndex,
    "the card should be repainted after the terminal shift",
  );
...
test("terminal split keeps the navigation card visible while wheel scrolling is active", (t) => {
  t.mock.timers.enable({ apis: ["setTimeout"] });
  ...
  assert.equal(terminal.writes.length, 1);
  assert.match(terminal.writes[0] ?? "", /Jump to bottom/);
  assert.ok(tui.render(80).some((line) => line.includes("Jump to bottom")));
  ...
  assert.ok(paint.indexOf("Jump to bottom") > paint.indexOf("\x1b[3T"));
...
test("terminal split renders a scroll-away navigation card in render and repaint paths", () => {
  ...
  assert.deepEqual(inputListener?.(`\x1b[<0;${bottomCol};${bottomRowIndex + 1}M`), {
    consume: true,
  });
  assert.ok(!tui.render(80).some((line) => line.includes("Jump to bottom")));
...
test("terminal split routes every scroll-away shortcut card click to bottom", () => {
  ...
  const bottomBorderRowIndex = rendered.findIndex(
  ...
  assert.deepEqual(inputListener?.(`\x1b[<0;${lastCol + 1};${rowFor("Jump to bottom")}M`), {
    consume: true,
  });

  assert.deepEqual(
    bottomClicks,
    clickTargets.map(() => "bottom"),
  );
...
test("terminal split scroll-away navigation card width tiers collapse without wrapping", () => {
  const cases = [
    {
      width: 80,
      includes: ["Jump to bottom", "User messages", "Assistant responses"],
      excludes: [],
      clickText: "┌",
    },
    {
      width: 50,
      includes: ["Bottom", "User", "Assistant", "prev ctrl+shift+u"],
      excludes: ["Jump to bottom"],
      clickText: "┌",
    },
    {
      width: 30,
      includes: ["User prev/next", "⌃⇧U/I", "Asst prev/next", "⌃⌥,/."],
      excludes: ["User messages"],
      clickText: "┌",
    },
    {
      width: 20,
      includes: ["Bottom ctrl+alt+g ↓"],
      excludes: ["User prev/next"],
      clickText: "Bottom ctrl+alt+g ↓",
    },
  ...
  for (const { width, includes, excludes, clickText } of cases) {
    ...
    if (clickText) {
      ...
      if (start + clickWidth < width) {
        assert.deepEqual(inputListener?.(`\x1b[<0;${start + clickWidth + 1};${rowIndex + 1}M`), {
          consume: true,
        });
      } else {
...
test("terminal split guards wrapped terminal writes", () => {
  ...
  assert.ok(
    write.lastIndexOf("\x1b[?1002h\x1b[?1006h") > write.lastIndexOf("\x1b[?1006l\x1b[?1002l"),
  );
...
class PrototypeTui {
  ...
  compositeLineAt(baseLine: string, overlayLine: string): string {
    this.compositeLineAtCalls += 1;
    return `${baseLine}|${overlayLine}`;
  }
}

/** Mirrors Pi 0.84's `createInteractiveTuiReference` so renderer swaps stay transparent. */
function createTuiReference(getTui: () => object): any {
  return new Proxy(
    {},
    {
...
test("terminal split delegates through Pi's dynamic TUI reference proxy", () => {
  const terminal = new FakeTerminal();
  const renderer = new PrototypeTui(terminal);
  const tui = createTuiReference(() => renderer);
  ...
  // Pi calls the proxy rather than the renderer. The proxy re-reads the method
  // on every call, so delegating to a reference captured through it would make
  // the installed override invoke itself until the stack overflowed.
  tui.doRender();

  assert.equal(renderer.doRenderCalls, 1);
  assert.ok(terminal.writes.some((write) => write.includes("cluster")));

  assert.equal(tui.compositeLineAt("base", "overlay", 0, 1, 1), "base|overlay");
  assert.equal(renderer.compositeLineAtCalls, 1);

  compositor.dispose();

  terminal.writes = [];
  tui.doRender();
  assert.equal(renderer.doRenderCalls, 2);
  assert.ok(terminal.writes.some((write) => write.includes("body")));
});

test("terminal split keeps delegating after the TUI reference swaps renderers", () => {
  const terminal = new FakeTerminal();
  let renderer = new PrototypeTui(terminal);
  const tui = createTuiReference(() => renderer);
  ...
  const replacement = new PrototypeTui(terminal);
  renderer = replacement;

  terminal.writes = [];
  tui.doRender();

  assert.equal(replacement.doRenderCalls, 1);
  assert.ok(terminal.writes.some((write) => write.includes("cluster")));

  compositor.dispose();
});

test("terminal split reattaches renderer hooks after the TUI reference swaps", () => {
  const terminal = new FakeTerminal();
  let renderer = new PrototypeTui(terminal);
  const tui = createTuiReference(() => renderer);
  ...
  // install() writes the hooks through Pi's reference, which forwards assignment
  // to the renderer that is current at that moment.
  assert.ok(Object.hasOwn(renderer, "doRender"));
  assert.ok(Object.hasOwn(renderer, "render"));
  assert.ok(Object.hasOwn(renderer, "compositeLineAt"));

  // A TUI mode change builds a replacement renderer, so it starts with none of
  // them. Asserting on terminal.writes would not catch this: the compositor also
  // patches the terminal, and Pi reuses the terminal across the swap.
  const replacement = new PrototypeTui(terminal);
  renderer = replacement;
  assert.ok(!Object.hasOwn(replacement, "doRender"));

  // Reading rows is enough to notice the swap, so the first frame after it is
  // already composited.
  void terminal.rows;

  assert.ok(Object.hasOwn(replacement, "doRender"), "doRender hook must be reattached");
  assert.ok(Object.hasOwn(replacement, "render"), "render hook must be reattached");
  assert.ok(
    Object.hasOwn(replacement, "compositeLineAt"),
    "compositeLineAt hook must be reattached",
  );
  ...
  // Delegation still reaches the replacement's own prototype method rather than
  // looping through the reattached hook.
  assert.equal(tui.compositeLineAt("base", "overlay", 0, 1, 1), "base|overlay");
  assert.equal(replacement.compositeLineAtCalls, 1);

  compositor.dispose();
  assert.equal(Reflect.get(replacement, RENDERER_HOOK_MARKER), false);
});

test("terminal split chains an own-property render override from another extension", () => {
  const terminal = new FakeTerminal();
  let outerCalls = 0;
  const tui = {
    terminal,
  ...
  const inner = tui.doRender.bind(tui);
  tui.doRender = () => {
    outerCalls += 1;
    inner();
  };
