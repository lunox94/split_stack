...
const SAFE_TASK_GIT_COMMANDS = new Map<string, string>([
  /* SCAFFOLD_TASK_ALLOWLIST */
]);

// Split Stack exposes no project Task wrappers around Git. Keep this map empty;
// application and agent tasks are passed through without translation.
...
// Optional project-local bridge for explicitly approved read-only task wrappers.
// Do not translate mutating git wrappers such as add, commit, push, or rebase.
function rewriteSafeTaskGitWrapper(cmd: string): string | null {
  const trimmed = cmd.trim();
  if (/\r|\n/.test(trimmed)) return null;

  const match = trimmed.match(
    /^task\s+(git:status|git:s|git:diff|git:d|git:log|git:l)(?:\s+--(?<args>.*))?$/,
  );
  ...
  const gitSubcommand = SAFE_TASK_GIT_COMMANDS.get(match[1]);
  if (!gitSubcommand) return null;

  return args === "" ? `git ${gitSubcommand}` : `git ${gitSubcommand} ${args}`;
}
...
export default async function (pi: ExtensionAPI) {
  ...
  pi.on("tool_call", async (event, ctx) => {
    try {
      ...
      const commandForRtk = rewriteSafeTaskGitWrapper(cmd) ?? cmd;

      // Delegate to RTK.
      const rewritten = await rewriteCommand(pi, commandForRtk, ctx.signal);
