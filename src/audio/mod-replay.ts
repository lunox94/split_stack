...
// @ts-nocheck -- minimally adapted vendored replay core; typed public seam below.
/*
	JavaScript ProTracker Replay (c)2017 mumart@gmail.com

	Source: https://github.com/martincameron/micromod/blob/83b5d9aebddb528ea177e115b0afed5d75f6b92a/micromod-js/Micromod.js
	Upstream blob: 2afe964f4304718071bfbb1483df96724cfeb8c6
...
function Micromod( module, samplingRate ) {
	...
	var downsample = function( buf, count ) {
		...
		var outLen = count * 2;
		for( var inIdx = 0, outIdx = 0; outIdx < outLen; inIdx += 4, outIdx += 2 ) {
...
export type ModReplayData = ArrayBuffer | ArrayBufferView;

const readModuleBytes = (data: ModReplayData): Int8Array => {
	if (data instanceof ArrayBuffer) return new Int8Array(data);
	return new Int8Array(data.buffer, data.byteOffset, data.byteLength);
};

const supportedChannelCount = (bytes: Int8Array): number => {
	...
	const oneDigit = /^([1-9])CHN$/.exec(signature);
	if (oneDigit !== null) return Number(oneDigit[1]);
	const twoDigit = /^(\d{2})CH$/.exec(signature);
	if (twoDigit !== null) return Number(twoDigit[1]);
	throw new Error("MOD format not recognised");
};

const validateModuleBytes = (bytes: Int8Array): void => {
	if (bytes.byteLength < 1084) throw new Error("MOD data is truncated");
	const channels = supportedChannelCount(bytes);
	if (channels < 1 || channels > 32) throw new Error("MOD channel count is unsupported");
	const sequenceLength = bytes[950] & 0x7f;
	if (sequenceLength < 1 || sequenceLength > 128) {
		throw new Error("MOD sequence length is invalid");
	}
	let patternCount = 0;
	for (let index = 0; index < 128; index += 1) {
		patternCount = Math.max(patternCount, (bytes[952 + index] & 0x7f) + 1);
	}
	const patternEnd = 1084 + patternCount * 64 * channels * 4;
	let sampleBytes = 0;
	for (let instrument = 1; instrument <= 31; instrument += 1) {
		const offset = instrument * 30 + 12;
		sampleBytes += (((bytes[offset] & 0xff) << 8) | (bytes[offset + 1] & 0xff)) * 2;
	}
	if (patternEnd + sampleBytes > bytes.byteLength) {
		throw new Error("MOD data is truncated");
	}
};

const assertSampleRate = (samplingRate: number): number => {
	if (!Number.isFinite(samplingRate) || samplingRate < 8000 || samplingRate > 128000) {
		throw new RangeError("MOD sampling rate must be between 8000 and 128000 Hz");
	}
	return Math.floor(samplingRate);
};

const assertOutputBuffers = (left: Float32Array, right: Float32Array): void => {
	if (!(left instanceof Float32Array) || !(right instanceof Float32Array)) {
		throw new TypeError("MOD output buffers must be Float32Array instances");
	}
	if (left.length !== right.length) {
		throw new RangeError("MOD output buffers must have the same length");
	}
};

/**
 * Stateful, synchronous ProTracker replay.
 *
 * Callers can render small PCM chunks directly into Web Audio buffers without
 * retaining a decoded copy of the whole song.
 */
export class ModReplay {
	...
	constructor(data: ModReplayData, samplingRate: number) {
		const bytes = readModuleBytes(data);
		validateModuleBytes(bytes);
		this.samplingRate = assertSampleRate(samplingRate);
		let parsed: Module;
		try {
			parsed = new Module(bytes);
		} catch (error) {
			throw new Error(
			`Cannot parse MOD data: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
		this.songName = parsed.songName.replace(/\s+$/, "");
		this.#replay = new Micromod(parsed, this.samplingRate);
		this.durationSamples = this.#replay.calculateSongDuration();
	...
	render(left: Float32Array, right: Float32Array): void {
		assertOutputBuffers(left, right);
		if (left.length === 0) return;
		this.#replay.getAudio(left, right, left.length);
		this.#positionSamples =
			(this.#positionSamples + left.length) % this.durationSamples;
	}
	...
	seek(samplePosition: number): number {
		if (!Number.isFinite(samplePosition)) {
			throw new RangeError("MOD seek position must be finite");
		}
		const target = Math.max(
			0,
			Math.min(Math.floor(samplePosition), Math.max(0, this.durationSamples - 1)),
		);
		const reached = this.#replay.seek(target);
		let remaining = target - reached;
		const scratchSize = Math.min(8192, Math.max(1, remaining));
		const left = new Float32Array(scratchSize);
		const right = new Float32Array(scratchSize);
		while (remaining > 0) {
			const count = Math.min(remaining, scratchSize);
			this.#replay.getAudio(
				count === scratchSize ? left : left.subarray(0, count),
				count === scratchSize ? right : right.subarray(0, count),
				count,
			);
			remaining -= count;
		}
		this.#positionSamples = target;
		return target;
	}
}
