...
export function parseNetworkTelemetrySummary(
  ...
  const { channel, receive, sinceAuthenticated, pump, snapshots, critical } = value;
  if (
    !isRecord(channel) ||
    !isRecord(receive) ||
...
export class NetworkTelemetry {
  ...
  private adjustSentKind(kind: MessageKind, amount: 1 | -1): void {
    if (kind === "SNAPSHOT") {
